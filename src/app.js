...
app.get('/admin-dashboard', (req, res) => {
  ...
  <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Admin Dashboard - Inspiration Capital Management Limited</title>
      <link rel="stylesheet" href="styles-modern.css">
  </head>
  <body>
      <div class="container">
          ...
          <nav style="display: flex; gap: var(--spacing-4); padding: var(--spacing-4); background: var(--gs-neutral-200); border-radius: var(--radius); margin-bottom: var(--spacing-6);">
              <a href="/admin-dashboard" class="btn btn-primary" style="text-decoration: none;">Dashboard</a>
              <a href="/admin-restricted-stocks" class="btn btn-secondary" style="text-decoration: none;">Restricted Stocks</a>
              <a href="/admin-requests" class="btn btn-secondary" style="text-decoration: none;">Trading Requests</a>
...
app.get('/employee-dashboard', (req, res) => {
  ...
  // Generate employee dashboard HTML
  const dashboardHTML = `
  <!DOCTYPE html>
  <html lang="en">
  <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Employee Dashboard - Inspiration Capital Management Limited</title>
      <link rel="stylesheet" href="styles-modern.css">
  </head>
  <body>
      <div class="container">
          ...
          <nav style="display: flex; gap: var(--spacing-4); padding: var(--spacing-4); background: var(--gs-neutral-200); border-radius: var(--radius);">
              <a href="/employee-dashboard" class="btn btn-primary" style="text-decoration: none;">Submit Request</a>
...
app.post('/preview-trade', async (req, res) => {
...
app.get('/admin-restricted-stocks', async (req, res) => {
  ...
  try {
    ...
    let tableRows = '';
    if (stocks.length === 0) {
      tableRows = '<tr><td colspan="4" style="text-align: center; padding: var(--spacing-6); color: var(--gs-neutral-600);">No restricted stocks found</td></tr>';
    } else {
      tableRows = stocks.map(stock => `
        <tr>
    ...
    const restrictedStocksHTML = `
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Restricted Stocks Management - Inspiration Capital Management Limited</title>
        <link rel="stylesheet" href="styles-modern.css">
    </head>
    <body>
        <div class="container">
            ...
            <nav style="display: flex; gap: var(--spacing-4); padding: var(--spacing-4); background: var(--gs-neutral-200); border-radius: var(--radius); margin-bottom: var(--spacing-6);">
                ...
                <a href="/admin-restricted-stocks" class="btn btn-primary" style="text-decoration: none;">Restricted Stocks</a>
                <a href="/admin-requests" class="btn btn-secondary" style="text-decoration: none;">Trading Requests</a>
                <a href="/admin-logout" class="btn btn-secondary" style="text-decoration: none;">Logout</a>
            </nav>
            ...
            <main>
                ...
                <div class="card" style="margin-bottom: var(--spacing-6);">
                    <div class="card-header">
                        <h3 class="card-title">Add New Restricted Stock</h3>
...
// Helper function to generate notification banners
  ...
  if (query.error) {
    ...
    switch (query.error) {
      case 'ticker_required':
        errorMessage = 'Stock ticker is required';
        break;
      case 'invalid_ticker':
        errorMessage = 'Invalid ticker format';
        errorDetail = query.ticker ? `"${decodeURIComponent(query.ticker)}" is not a valid ticker format. Please use formats like AAPL, 0001.HK, SAP.DE, etc.` : '';
        break;
      case 'stock_already_exists':
        errorMessage = 'Stock already exists';
        errorDetail = query.ticker ? `${decodeURIComponent(query.ticker)} is already in the restricted stocks list.` : '';
        break;
      case 'add_failed':
        errorMessage = 'Failed to add stock';
        errorDetail = query.details ? decodeURIComponent(query.details) : 'An unexpected error occurred while adding the stock.';
        break;
      case 'remove_failed':
        errorMessage = 'Failed to remove stock';
        break;
      case 'update_failed':
        errorMessage = 'Failed to update company names';
        break;
      default:
    ...
    return `
      <div style="margin-bottom: var(--spacing-6); padding: var(--spacing-4); background: #f8d7da; border: 1px solid #f5c6cb; border-radius: var(--radius);">
        <p style="margin: 0; color: #721c24;">
          <strong>❌ ${errorMessage}</strong>
          ${errorDetail ? `<br><span style="font-size: var(--font-size-sm);">${errorDetail}</span>` : ''}
        </p>
      </div>
    `;
    ...
    switch (query.message) {
      case 'stock_added':
        ...
        if (query.ticker && query.company) {
          successDetail = `${decodeURIComponent(query.ticker)} (${decodeURIComponent(query.company)}) has been added to the restricted stocks list.`;
        }
        break;
      case 'stock_removed':
        ...
        break;
      default:
        if (query.message.startsWith('updated_')) {
          const count = query.message.split('_')[1];
          successMessage = 'Company names updated';
          successDetail = `Updated ${count} stock entries with proper company names.`;
        } else {
    ...
    return `
      <div style="margin-bottom: var(--spacing-6); padding: var(--spacing-4); background: #d4edda; border: 1px solid #c3e6cb; border-radius: var(--radius);">
        <p style="margin: 0; color: #155724;">
          <strong>✅ ${successMessage}</strong>
          ${successDetail ? `<br><span style="font-size: var(--font-size-sm);">${successDetail}</span>` : ''}
        </p>
      </div>
    `;
  }
  ...
  try {
    ...
    for (const stock of stocksToUpdate) {
      ...
      await new Promise((resolve, reject) => {
        const sql = 'UPDATE restricted_stocks SET company_name = ? WHERE ticker = ?';
        db.run(sql, [companyName, stock.ticker], function(err) {
          if (err) reject(err);
          else {
...
app.get('/admin-requests', async (req, res) => {
  ...
  try {
    ...
    let tableRows = '';
    if (requests.length === 0) {
      tableRows = '<tr><td colspan="10" style="text-align: center; padding: var(--spacing-6); color: var(--gs-neutral-600);">No trading requests found</td></tr>';
    } else {
      tableRows = requests.map(request => {
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Trading Requests - Inspiration Capital Management Limited</title>
        <link rel="stylesheet" href="styles-modern.css">
    </head>
    <body>
        <div class="container">
